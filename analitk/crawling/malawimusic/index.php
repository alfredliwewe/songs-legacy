	...
	if($text != ""){
    	...
    	if ($links2['artist'] != "") {
	    	...
	    	if ($check->num_rows == 0) {
	    		...
	    		$artist_link = $db->real_escape_string($artist_data['artist']);
	    
	    		$ins = $db->query("INSERT INTO `mw_artists`(`id`, `webid`, `name`, `photo`, `biography`, `status`, `link`) VALUES (NULL, '{$artist_data['webid']}', '$artist_name', '$artist_photo', '', 'active', '$artist_link')");
	    	    ...
	    	    echo "Already added ".$data['title'];
	    	}
	    }
	    else{
	        echo "Artist link not found";
	    }
	}
