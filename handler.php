...
elseif (isset($_GET['getProductsHome']) OR isset($_POST['getProductsHome'])) {
	...
	$read = $db->query("SELECT * FROM `genre` ");
	$genres = [];
	$genre_store = [];
	$genre_list  = [];
	while ($row = $read->fetch_assoc()) {
	...
	//read songs played
	$sql = $db->query("SELECT genre FROM song_plays WHERE song_plays.user = '$user' ");
	while($row = $sql->fetch_assoc()){
	...
	echo json_encode(['mega' => $mega, 'adverts' => $adverts, 'genres' => $genre_list]);
}
...
elseif (isset($_GET['searchModal'])) {
	...
	$read = $db->query("SELECT web_songs.id AS song_id,title,name FROM web_songs JOIN web_artists ON web_songs.artist = web_artists.id WHERE name LIKE '%{$search}%' LIMIT 20");
	while ($row = $read->fetch_assoc()) {
		$row['type'] = 'malawi';
		array_push($data, $row);
	}
